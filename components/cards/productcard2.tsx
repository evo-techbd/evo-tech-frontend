...
const ProductCard2 = ({ eachItem }: { eachItem: any }) => {
  ...
  const isLowStock =
    eachItem.i_stock !== undefined &&
    eachItem.i_stock <= threshold &&
    eachItem.i_stock > 0;
    ...
    <div className="flex flex-col items-center w-full min-w-[150px] max-[550px]:max-w-[240px] max-w-[270px] h-fit bg-[#FFFFFF] overflow-hidden rounded-[10px] shadow-sm shadow-black/15">
      <div className="flex flex-col items-center w-full min-h-[330px] h-[330px]">
            ...
            <div className="relative w-full h-full rounded-t-[4px] overflow-hidden bg-white">
              ...
              {isLowStock && (
                <div
                  ...
                  title={`Only ${eachItem.i_stock} left in stock`}
                />
              )}
            </div>
          </Link>
        ...
        <div className="w-full h-full flex flex-col px-4 py-3 border-t border-stone-200">
          ...
          <div className="w-full text-left flex items-center gap-2 flex-wrap font-[500] text-[13px] leading-6 text-stone-700 tracking-tight">
            {eachItem.i_instock ? (
              <>
                <span>{`BDT ${currencyFormatBDT(eachItem.i_price)}`}</span>
                {eachItem.i_prevprice !== 0 && (
                  <span className="text-[13px] text-stone-500 font-[500] line-through decoration-1 decoration-stone-600">
                    {currencyFormatBDT(eachItem.i_prevprice)}
                  </span>
                )}
              </>
            ) : (
      ...
      <div className="flex flex-col items-center w-full h-fit p-2 gap-2">
        {eachItem.i_subcategory === "filaments" ? (
          <Link
            href={`/items/${eachItem.i_slug}`}
            className={`w-full h-fit px-5 py-2 text-center text-[12px] sm:text-[13px] leading-5 font-[600] tracking-tight text-stone-100 bg-stone-900 hover:bg-stone-700 rounded-[6px] overflow-hidden transition-colors duration-100 ease-linear focus:outline-none`}
          >
            View Details
          </Link>
        ) : (
          <button
            ...
            disabled={!eachItem.i_instock && !eachItem.i_ispreorder}
            className="w-full h-fit px-5 py-2 text-center text-[12px] sm:text-[13px] leading-5 font-[600] tracking-tight text-stone-100 bg-stone-900 disabled:bg-stone-600 hover:bg-stone-700 rounded-[6px] overflow-hidden transition-colors duration-100 ease-linear"
          >
            Add to Cart
        ...
        <button
          ...
          className="w-full h-fit px-5 py-2 text-center text-[12px] sm:text-[13px] leading-5 font-[600] tracking-tight text-stone-700 border border-stone-200 rounded-[6px] overflow-hidden hover:bg-stone-200 transition-colors duration-100 ease-linear"
        >
          Add to Wishlist

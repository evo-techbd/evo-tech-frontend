...
const getActiveContent = async (): Promise<ContentData | null> => {
    try {
        const response = await axios.get(`/page-content/student-discount/active`);
        if (response.data && response.data.data) {
            return response.data.data;
        }
        return null;
    } catch (error: any) {
        console.error("Error fetching student discount:", error.message);
        return null;
    }
};

const StudentDiscount = async () => {
    let contentData: ContentData | null = null;

    try {
        contentData = await getActiveContent();
    } catch (error) {
        console.error("Student discount page error:", error);
    }
                    ...
                    <Card className="shadow-lg">
                        <CardContent className="py-12 text-center">
                            ...
                            <p className="text-stone-600 text-lg">
                                Student discount information is currently being updated. Please
                                check back later.
                            </p>
                        </CardContent>
                    </Card>

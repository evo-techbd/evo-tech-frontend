...
const getActiveContent = async (): Promise<ContentData | null> => {
    try {
        const response = await axios.get(`/page-content/shop-shape-tomorrow/active`);
        if (response.data && response.data.data) {
            return response.data.data;
        }
        return null;
    } catch (error: any) {
        console.error("Error fetching shop & shape tomorrow:", error.message);
        return null;
    }
};

const ShopShapeTomorrow = async () => {
    let contentData: ContentData | null = null;

    try {
        contentData = await getActiveContent();
    } catch (error) {
        console.error("Shop & shape tomorrow page error:", error);
    }
...
export default ShopShapeTomorrow;
